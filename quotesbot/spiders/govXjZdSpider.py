...
class GovXjZdBid(scrapy.Spider):
    name = "govXjZd"
    ...
    # url_xj_zd指定时间段
    url_xj_zd = baseUrl + paraS + "2" + paraM + zdTime + paraE
    ...
    def __init__(self):
        ...
        dispatcher.connect(receiver=self.GovXjZdBidCloseHandle,
                           signal=signals.spider_closed
                           )

    # 信号量处理函数：关闭chrome浏览器
    def GovXjZdBidCloseHandle(self, spider):
        print(f"GovXjZdBidCloseHandle: enter ")
        self.driver.quit()
        ...
        try:
            ...
            if next != "" and next is not None:
                ...
                if int(nextPageNum) >= 1:
                    next_url = self.url_xj_zd.replace("page_index=1", "page_index=" + nextPageNum)
