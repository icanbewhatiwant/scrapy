...
class SeleniumSpider(scrapy.Spider):
    ...
    # start_time = (today - relativedelta(days=+5)).strftime('%Y:%m:%d')
    ...
    # 信号量处理函数：关闭chrome浏览器
    def SeleniumSpiderCloseHandle(self, spider):
        print(f"SeleniumSpiderCloseHandle: enter ")
        self.driver.quit()
        ...
        try:
            ...
            if next != "" and next is not None:
                ...
                if int(nextPageNum) >= 1:
                    ...
                    next_url = self.url_gk_today.replace("page_index=1", "page_index=" + nextPageNum)
