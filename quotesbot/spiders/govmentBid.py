...
class GovmentBid(scrapy.Spider):
    name = "govBidxj"
    ...
    def __init__(self):
        ...
        dispatcher.connect(receiver=self.GovBidSpiderCloseHandle,
                           signal=signals.spider_closed
                           )

    # 信号量处理函数：关闭chrome浏览器
    def GovBidSpiderCloseHandle(self, spider):
        print(f"GovBidSpiderCloseHandle: enter ")
        self.driver.quit()
        ...
        try:
            ...
            if next != "" and next is not None:
                ...
                if int(nextPageNum) >= 1:
                    next_url = self.url_xj_today.replace("page_index=1", "page_index=" + nextPageNum)
